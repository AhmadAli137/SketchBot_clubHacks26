...
export function AiboticsLogo({ size = 40, showWordmark = true, className, animate = true }: LogoProps) {
  const s = size;
  // Mark is a small accent — lettering is the hero
  const markSize = Math.round(s * 0.62);

  return (
    <div
      className={`aibotics-logo-root ${className ?? ''}`}
      style={{ display: 'flex', alignItems: 'center', gap: Math.round(s * 0.18) }}
    >
      {/* ── Mark: compact robot-face badge ── */}
      <motion.svg
        width={markSize} height={markSize} viewBox="0 0 28 28" fill="none"
      ...
      {showWordmark && (
        ...
        >
          ...
          <div
            className="aibotics-wordmark-sub"
            style={{ fontSize: Math.round(s * 0.24), letterSpacing: '0.10em', fontWeight: 500 }}
          >
            AI ROBOTICS STUDIO
          </div>
        </motion.div>
