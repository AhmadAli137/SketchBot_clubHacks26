...
public class WebRTCView extends ViewGroup {
     ...
     */
    private final View rendererView;
    private final Object videoRenderer;
    ...
    public WebRTCView(Context context) {
        ...
        videoRenderer = createVideoRenderer(context);
        rendererView = (View) videoRenderer;
    ...
    private Object createVideoRenderer(Context context) {
        try {
            Class<?> textureRendererClass = Class.forName("org.webrtc.TextureViewRenderer");
            return textureRendererClass.getConstructor(Context.class).newInstance(context);
        } catch (Throwable ignored) {
            return new SurfaceViewRenderer(context);
        }
    }

    private void invokeRenderer(String methodName) {
        try {
            Method method = videoRenderer.getClass().getMethod(methodName);
            method.invoke(videoRenderer);
        } catch (NoSuchMethodException ignored) {
        } catch (IllegalAccessException | InvocationTargetException e) {
            Log.w(TAG, "Failed to invoke renderer method " + methodName, e);
        }
    }

    private void invokeRenderer(String methodName, Class<?> parameterType, Object argument) {
        try {
            Method method = videoRenderer.getClass().getMethod(methodName, parameterType);
            method.invoke(videoRenderer, argument);
        } catch (NoSuchMethodException ignored) {
        } catch (IllegalAccessException | InvocationTargetException e) {
            Log.w(TAG, "Failed to invoke renderer method " + methodName, e);
        }
        ...
        if (rendererAttached) {
            if (videoTrack != null) {
                ThreadUtils.runOnExecutor(() -> {
                    try {
                        videoTrack.removeSink((VideoSink) videoRenderer);
                    } catch (Throwable tr) {
    ...
    public void setMirror(boolean mirror) {
        if (this.mirror != mirror) {
            this.mirror = mirror;
            invokeRenderer("setMirror", boolean.class, mirror);
    ...
    private void setScalingType(ScalingType scalingType) {
        synchronized (layoutSyncRoot) {
            ...
            invokeRenderer("setScalingType", ScalingType.class, scalingType);
        }
    ...
    public void setZOrder(int zOrder) {
        switch (zOrder) {
            case 0:
                invokeRenderer("setZOrderMediaOverlay", boolean.class, false);
                invokeRenderer("setZOrderOnTop", boolean.class, false);
                break;
            case 1:
                invokeRenderer("setZOrderMediaOverlay", boolean.class, true);
                invokeRenderer("setZOrderOnTop", boolean.class, false);
                break;
            case 2:
                invokeRenderer("setZOrderOnTop", boolean.class, true);
                break;
        }
    }
    ...
    private void tryAddRendererToVideoTrack() {
        if (!rendererAttached && videoTrack != null && ViewCompat.isAttachedToWindow(this)) {
            ...
            try {
                invokeRenderer("init", EglBase.Context.class, sharedContext, RendererEvents.class, rendererEvents);
                surfaceViewRendererInstances++;
            } catch (Exception e) {
                Logging.e(
            ...
            ThreadUtils.runOnExecutor(() -> {
                try {
                    videoTrack.addSink((VideoSink) videoRenderer);
                } catch (Throwable tr) {
                    // XXX If WebRTCModule#mediaStreamTrackRelease has already been
    ...
    private void invokeRenderer(String methodName, Class<?> firstParameterType, Object firstArgument, Class<?> secondParameterType, Object secondArgument) {
        try {
            Method method = videoRenderer.getClass().getMethod(methodName, firstParameterType, secondParameterType);
            method.invoke(videoRenderer, firstArgument, secondArgument);
        } catch (NoSuchMethodException ignored) {
        } catch (IllegalAccessException | InvocationTargetException e) {
            Log.w(TAG, "Failed to invoke renderer method " + methodName, e);
        }
